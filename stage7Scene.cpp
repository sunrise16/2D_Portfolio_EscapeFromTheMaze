...
HRESULT stage7Scene::init()
{
	...
	// 이미지 초기화
	setStage(IMAGEMANAGER->findImage("Map_Stage7"));
...
void stage7Scene::update()
{
	...
	// 스테이지 인식하게 하기
	getPlayer()->setCurrentStage(7);
	getClone()->setCurrentStage(7);
	getObject()->setCurrentStage(7);

	// 트리거 작동
	RECT rc1;
	// 게이트 스위치 작동 처리
	// 게이트 스위치 1
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[14].objectRect))
	{
		if (getObject()->getObject()[14].switchSoundOutput == FALSE)
		{
			SOUNDMANAGER->play("Switch");
			getObject()->setSwitchSoundOutput(14, TRUE);
		}
		getObject()->setSwitchOn(14, TRUE);
	}
	for (int i = 0; i < getClone()->getClone().size(); i++)
	{
		if (IntersectRect(&rc1, &getClone()->getClone()[i].cloneRect, &getObject()->getObject()[14].objectRect))
		{
			if (getObject()->getObject()[14].switchSoundOutput == FALSE)
			{
				SOUNDMANAGER->play("Switch");
	...
	if (getObject()->getObject()[14].switchOn == TRUE &&
		(!IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[14].objectRect)))
	{
		if (getClone()->getClone().empty())
		{
			getObject()->setSwitchOff(14, TRUE);

			if (getObject()->getObject()[14].switchOffDelay >= 30)
			{
				getObject()->setSwitchSoundOutput(14, FALSE);
				getObject()->setSwitchOn(14, FALSE);
				getObject()->setSwitchOff(14, FALSE);
			}
		}
		else
		{
	...
	// 게이트 스위치 2
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[15].objectRect))
	{
		if (getObject()->getObject()[15].switchSoundOutput == FALSE)
		{
			SOUNDMANAGER->play("Switch");
			getObject()->setSwitchSoundOutput(15, TRUE);
		}
		getObject()->setSwitchOn(15, TRUE);
	}
	for (int i = 0; i < getClone()->getClone().size(); i++)
	{
		if (IntersectRect(&rc1, &getClone()->getClone()[i].cloneRect, &getObject()->getObject()[15].objectRect))
		{
			if (getObject()->getObject()[15].switchSoundOutput == FALSE)
			{
				SOUNDMANAGER->play("Switch");
	...
	if (getObject()->getObject()[15].switchOn == TRUE &&
		(!IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[15].objectRect)))
	{
		if (getClone()->getClone().empty())
		{
			getObject()->setSwitchOff(15, TRUE);

			if (getObject()->getObject()[15].switchOffDelay >= 30)
			{
				getObject()->setSwitchSoundOutput(15, FALSE);
				getObject()->setSwitchOn(15, FALSE);
				getObject()->setSwitchOff(15, FALSE);
			}
		}
		else
		{
	...
	// 게이트 스위치 전부 작동 시 게이트 활성화
	if (getObject()->getObject()[14].switchOn == TRUE && getObject()->getObject()[15].switchOn == TRUE)
	{
		...
		getObject()->setSwitchOn(22, TRUE);
	}
	else
	{
		setOpenSoundOutput(FALSE);
		getObject()->setSwitchOn(22, FALSE);
		getObject()->setSwitchOff(22, FALSE);
	}
	// 흰색 타일 스위치 작동 처리
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[16].objectRect))
	{
		if (getObject()->getObject()[16].switchSoundOutput == FALSE)
		{
			SOUNDMANAGER->play("Switch");
			getObject()->setSwitchSoundOutput(16, TRUE);
		}
		getObject()->setSwitchOn(16, TRUE);
		for (int i = 0; i < 6; i++)
		{
	...
	for (int i = 0; i < getClone()->getClone().size(); i++)
	{
		if (IntersectRect(&rc1, &getClone()->getClone()[i].cloneRect, &getObject()->getObject()[16].objectRect))
		{
			if (getObject()->getObject()[16].switchSoundOutput == FALSE)
			{
				SOUNDMANAGER->play("Switch");
	...
	if (getObject()->getObject()[16].switchOn == TRUE &&
		(!IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[16].objectRect)))
	{
		if (getClone()->getClone().empty())
		{
			getObject()->setSwitchOff(16, TRUE);
			if (getObject()->getObject()[16].switchOffDelay >= 30)
			{
				getObject()->setSwitchSoundOutput(16, FALSE);
				getObject()->setSwitchOn(16, FALSE);
				getObject()->setSwitchOff(16, FALSE);
				for (int i = 0; i < 6; i++)
				{
	...
	// 빨간색 타일 스위치 작동 처리
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[17].objectRect))
	{
		if (getObject()->getObject()[17].switchSoundOutput == FALSE)
		{
			SOUNDMANAGER->play("Switch");
			getObject()->setSwitchSoundOutput(17, TRUE);
		}
		getObject()->setSwitchOn(17, TRUE);
		for (int i = 6; i < 8; i++)
		{
			if (getObject()->getObject()[i].switchOn == FALSE)
			{
	...
	for (int i = 0; i < getClone()->getClone().size(); i++)
	{
		if (IntersectRect(&rc1, &getClone()->getClone()[i].cloneRect, &getObject()->getObject()[17].objectRect))
		{
			if (getObject()->getObject()[17].switchSoundOutput == FALSE)
			{
				SOUNDMANAGER->play("Switch");
	...
	if (getObject()->getObject()[17].switchOn == TRUE &&
		(!IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[17].objectRect)))
	{
		if (getClone()->getClone().empty())
		{
			getObject()->setSwitchOff(17, TRUE);
			if (getObject()->getObject()[17].switchOffDelay >= 30)
			{
				getObject()->setSwitchSoundOutput(17, FALSE);
				getObject()->setSwitchOn(17, FALSE);
				getObject()->setSwitchOff(17, FALSE);
				for (int i = 6; i < 8; i++)
				{
					if (getObject()->getObject()[i].switchOn == TRUE)
					{
	...
	// 스테이지 클리어 처리
	RECT rc2;
	if (IntersectRect(&rc2, &getPlayer()->getPlayerRect(), &getObject()->getObject()[22].objectRect) && getObject()->getObject()[22].switchOn == TRUE)
	{
	...
	if (INPUT->getKey(0x52) && getPlayer()->getDead() == FALSE)
	{
		...
		{
			getPlayer()->init(7, getAlpha());
			getClone()->init(getAlpha());
			getObject()->getObject().clear();
			getObject()->init(7, getAlpha());
			setStageRestartDelay(0);
		}
	}
