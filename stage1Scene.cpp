...
void stage1Scene::update()
{
	...
	// 스테이지 인식하게 하기
	getPlayer()->setCurrentStage(1);
	getClone()->setCurrentStage(1);
	getObject()->setCurrentStage(1);

	// 스테이지 클리어 처리
	RECT rc1;
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[1].objectRect))
	{
	...
	if (INPUT->getKey(0x52) && getPlayer()->getDead() == FALSE)
	{
		...
		{
			getPlayer()->init(1, getAlpha());
			getClone()->init(getAlpha());
			getObject()->getObject().clear();
			getObject()->init(1, getAlpha());
			setStageRestartDelay(0);
		}
	}
...
void stage1Scene::render()
{
	...
	// 메세지 출력
	RECT rc2;
	if (IntersectRect(&rc2, &getPlayer()->getPlayerRect(), &getObject()->getObject()[0].objectRect))
	{
	...
	if (!getClone()->getClone().empty())
	{
		sprintf_s(str, "clone_cloneDelay : %d", getClone()->getClone()[0].cloneDelay);
		TextOut(getMemDC(), 0, 360, str, strlen(str));
	}
}

// 애니메이션 (animation)
void stage1Scene::animation()
{
