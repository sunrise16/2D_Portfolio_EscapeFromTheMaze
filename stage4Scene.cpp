...
void stage4Scene::update()
{
	...
	// 스테이지 인식하게 하기
	getPlayer()->setCurrentStage(4);
	getClone()->setCurrentStage(4);
	getObject()->setCurrentStage(4);

	// 트리거 작동
	RECT rc1;
	// 게이트 스위치 작동 처리
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[11].objectRect))
	{
		...
		if (getObject()->getObject()[11].switchSoundOutput == FALSE)
		{
			SOUNDMANAGER->play("Switch");
			getObject()->setSwitchSoundOutput(11, TRUE);
		}
		getObject()->setSwitchOn(11, TRUE);
		getObject()->setSwitchOn(15, TRUE);
	}
	for (int i = 0; i < getClone()->getClone().size(); i++)
	{
		if (IntersectRect(&rc1, &getClone()->getClone()[i].cloneRect, &getObject()->getObject()[11].objectRect))
		{
			if (getOpenSoundOutput() == FALSE && !SOUNDMANAGER->isPlaySound("Open"))
			{
				SOUNDMANAGER->play("Open");
	...
	if (getObject()->getObject()[11].switchOn == TRUE &&
		(!IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[11].objectRect)))
	{
		if (getClone()->getClone().empty())
		{
			getObject()->setSwitchOff(11, TRUE);
			getObject()->setSwitchOff(15, TRUE);

			if (getObject()->getObject()[11].switchOffDelay >= 30)
			{
				getObject()->setSwitchSoundOutput(11, FALSE);
				getObject()->setSwitchOn(11, FALSE);
				getObject()->setSwitchOff(11, FALSE);
			}
	...
	// 빨간색 타일 스위치 작동 처리
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[12].objectRect))
	{
		if (getObject()->getObject()[12].switchSoundOutput == FALSE)
		{
			SOUNDMANAGER->play("Switch");
			getObject()->setSwitchSoundOutput(12, TRUE);
		}
		getObject()->setSwitchOn(12, TRUE);
		for (int i = 0; i < 3; i++)
		{
	...
	for (int i = 0; i < getClone()->getClone().size(); i++)
	{
		if (IntersectRect(&rc1, &getClone()->getClone()[i].cloneRect, &getObject()->getObject()[12].objectRect))
		{
			if (getObject()->getObject()[12].switchSoundOutput == FALSE)
			{
				SOUNDMANAGER->play("Switch");
	...
	if (getObject()->getObject()[12].switchOn == TRUE &&
		(!IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[12].objectRect)))
	{
		if (getClone()->getClone().empty())
		{
			getObject()->setSwitchOff(12, TRUE);
			if (getObject()->getObject()[12].switchOffDelay >= 30)
			{
				getObject()->setSwitchSoundOutput(12, FALSE);
				getObject()->setSwitchOn(12, FALSE);
				getObject()->setSwitchOff(12, FALSE);
				for (int i = 0; i < 3; i++)
				{
	...
	// 초록색 타일 스위치 작동 처리
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[13].objectRect))
	{
		if (getObject()->getObject()[13].switchSoundOutput == FALSE)
		{
			SOUNDMANAGER->play("Switch");
			getObject()->setSwitchSoundOutput(13, TRUE);
		}
		getObject()->setSwitchOn(13, TRUE);
		for (int i = 3; i < 6; i++)
		{
			if (getObject()->getObject()[i].switchOn == FALSE)
			{
	...
	for (int i = 0; i < getClone()->getClone().size(); i++)
	{
		if (IntersectRect(&rc1, &getClone()->getClone()[i].cloneRect, &getObject()->getObject()[13].objectRect))
		{
			getObject()->setSwitchOn(13, TRUE);
			for (int j = 3; j < 6; j++)
			{
	...
	if (getObject()->getObject()[13].switchOn == TRUE &&
		(!IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[13].objectRect)))
	{
		if (getClone()->getClone().empty())
		{
			getObject()->setSwitchOff(13, TRUE);
			if (getObject()->getObject()[13].switchOffDelay >= 30)
			{
				getObject()->setSwitchSoundOutput(13, FALSE);
				getObject()->setSwitchOn(13, FALSE);
				getObject()->setSwitchOff(13, FALSE);
				for (int i = 3; i < 6; i++)
				{
					if (getObject()->getObject()[i].switchOn == TRUE)
					{
	...
	// 스테이지 클리어 처리
	RECT rc2;
	if (IntersectRect(&rc2, &getPlayer()->getPlayerRect(), &getObject()->getObject()[15].objectRect) && getObject()->getObject()[15].switchOn == TRUE)
	{
	...
	if (INPUT->getKey(0x52) && getPlayer()->getDead() == FALSE)
	{
		...
		{
			getPlayer()->init(4, getAlpha());
			getClone()->init(getAlpha());
			getObject()->getObject().clear();
			getObject()->init(4, getAlpha());
			setStageRestartDelay(0);
		}
	}
...
void stage4Scene::render()
{
	...
	// 메세지 출력
	RECT rc3;
	if (IntersectRect(&rc3, &getPlayer()->getPlayerRect(), &getObject()->getObject()[14].objectRect))
	{
