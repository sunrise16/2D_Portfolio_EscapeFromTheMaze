...
HRESULT stage5Scene::init()
{
	...
	// 이미지 초기화
	setStage(IMAGEMANAGER->findImage("Map_Stage5"));
	setResetMessage(IMAGEMANAGER->findImage("System_ResetMessage"));
	setMessageBox(IMAGEMANAGER->findImage("System_MessageBox"));
	setSystemMessage1(IMAGEMANAGER->findImage("System_SystemMessage6"));
	setSystemMessage2(IMAGEMANAGER->findImage("System_SystemMessage7"));
...
void stage5Scene::update()
{
	...
	// 스테이지 인식하게 하기
	getPlayer()->setCurrentStage(5);
	getClone()->setCurrentStage(5);
	getObject()->setCurrentStage(5);

	// 스테이지 5 전용 재시작 처리 조건 만족시키기
	RECT rc1;
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[2].objectRect))
	{
		if (getStage5Restart() == FALSE)
		{
			setStage5Restart(TRUE);
		}
	}

	// 스테이지 클리어 처리
	RECT rc2;
	if (IntersectRect(&rc2, &getPlayer()->getPlayerRect(), &getObject()->getObject()[3].objectRect))
	{
	...
	if (INPUT->getKey(0x52) && getPlayer()->getDead() == FALSE)
	{
		...
		{
			if (getStage5Restart() == TRUE)
			{
				setInitCount(getInitCount() + 1);
			}
			getPlayer()->init(5, getAlpha());
			getClone()->init(getAlpha());
			getObject()->getObject().clear();
			if (getStage5Restart() == FALSE)
			{
				getObject()->init(5, getAlpha());
			}
			else
			{
				getObject()->initStage5(getInitCount(), getAlpha());
			}
			setStageRestartDelay(0);
		}
	}
...
void stage5Scene::render()
{
	...
	// 메세지 출력 1
	RECT rc3;
	if (IntersectRect(&rc3, &getPlayer()->getPlayerRect(), &getObject()->getObject()[1].objectRect))
	{
	...
	// 메세지 출력 2
	RECT rc4;
	if (IntersectRect(&rc4, &getPlayer()->getPlayerRect(), &getObject()->getObject()[2].objectRect))
	{
		getMessageBox()->render(getMemDC(), 0, WINSIZEY - 300);
