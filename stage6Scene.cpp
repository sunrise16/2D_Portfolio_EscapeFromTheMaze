...
HRESULT stage6Scene::init()
{
	...
	// 이미지 초기화
	setStage(IMAGEMANAGER->findImage("Map_Stage6"));
...
void stage6Scene::update()
{
	...
	// 스테이지 인식하게 하기
	getPlayer()->setCurrentStage(6);
	getClone()->setCurrentStage(6);
	getObject()->setCurrentStage(6);

	// 트리거 작동
	RECT rc1;
	// 게이트 스위치 작동 처리
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[6].objectRect))
	{
		...
		if (getObject()->getObject()[6].switchSoundOutput == FALSE)
		{
			SOUNDMANAGER->play("Switch");
			getObject()->setSwitchSoundOutput(6, TRUE);
		}
		getObject()->setSwitchOn(6, TRUE);
		getObject()->setSwitchOn(9, TRUE);
	}
	for (int i = 0; i < getClone()->getClone().size(); i++)
	{
		if (IntersectRect(&rc1, &getClone()->getClone()[i].cloneRect, &getObject()->getObject()[6].objectRect))
		{
			if (getOpenSoundOutput() == FALSE && !SOUNDMANAGER->isPlaySound("Open"))
			{
				SOUNDMANAGER->play("Open");
	...
	if (getObject()->getObject()[6].switchOn == TRUE &&
		(!IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[6].objectRect)))
	{
		if (getClone()->getClone().empty())
		{
			getObject()->setSwitchOff(6, TRUE);
			getObject()->setSwitchOff(9, TRUE);

			if (getObject()->getObject()[6].switchOffDelay >= 30)
			{
				getObject()->setSwitchSoundOutput(6, FALSE);
				getObject()->setSwitchOn(6, FALSE);
				getObject()->setSwitchOff(6, FALSE);
			}
	...
	// 흰색 타일 스위치 작동 처리
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[7].objectRect))
	{
	...
	// 빨간색 타일 스위치 작동 처리
	if (IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[8].objectRect))
	{
		if (getObject()->getObject()[8].switchSoundOutput == FALSE)
		{
			SOUNDMANAGER->play("Switch");
			getObject()->setSwitchSoundOutput(8, TRUE);
		}
		getObject()->setSwitchOn(8, TRUE);
		for (int i = 3; i < 6; i++)
		{
			if (getObject()->getObject()[i].switchOn == FALSE)
			{
	...
	for (int i = 0; i < getClone()->getClone().size(); i++)
	{
		if (IntersectRect(&rc1, &getClone()->getClone()[i].cloneRect, &getObject()->getObject()[8].objectRect))
		{
			if (getObject()->getObject()[8].switchSoundOutput == FALSE)
			{
				SOUNDMANAGER->play("Switch");
	...
	if (getObject()->getObject()[8].switchOn == TRUE &&
		(!IntersectRect(&rc1, &getPlayer()->getPlayerRect(), &getObject()->getObject()[8].objectRect)))
	{
		if (getClone()->getClone().empty())
		{
			getObject()->setSwitchOff(8, TRUE);
			if (getObject()->getObject()[8].switchOffDelay >= 30)
			{
				getObject()->setSwitchSoundOutput(8, FALSE);
				getObject()->setSwitchOn(8, FALSE);
				getObject()->setSwitchOff(8, FALSE);
				for (int i = 3; i < 6; i++)
				{
					if (getObject()->getObject()[i].switchOn == TRUE)
					{
	...
	if (INPUT->getKey(0x52) && getPlayer()->getDead() == FALSE)
	{
		...
		{
			getPlayer()->init(6, getAlpha());
			getClone()->init(getAlpha());
			getObject()->getObject().clear();
			getObject()->init(6, getAlpha());
			setStageRestartDelay(0);
		}
	}
...
void stage6Scene::render()
{
	...
	if (!getClone()->getClone().empty())
	{
		if (getClone()->getClone().size() == 1)
		{
			sprintf_s(str, "clone1_tileCollisionBottom : %d", getClone()->getClone()[0].tileCollisionBottom);
			TextOut(getMemDC(), 0, 520, str, strlen(str));
		}
		else if (getClone()->getClone().size() == 2)
		{
			sprintf_s(str, "clone1_tileCollisionBottom : %d", getClone()->getClone()[0].tileCollisionBottom);
			TextOut(getMemDC(), 0, 520, str, strlen(str));
			sprintf_s(str, "clone2_tileCollisionBottom : %d", getClone()->getClone()[1].tileCollisionBottom);
			TextOut(getMemDC(), 0, 540, str, strlen(str));
		}
		else if (getClone()->getClone().size() == 3)
		{
			sprintf_s(str, "clone1_tileCollisionBottom : %d", getClone()->getClone()[0].tileCollisionBottom);
			TextOut(getMemDC(), 0, 520, str, strlen(str));
			sprintf_s(str, "clone2_tileCollisionBottom : %d", getClone()->getClone()[1].tileCollisionBottom);
			TextOut(getMemDC(), 0, 540, str, strlen(str));
			sprintf_s(str, "clone3_tileCollisionBottom : %d", getClone()->getClone()[2].tileCollisionBottom);
